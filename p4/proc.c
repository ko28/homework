...
static node nodes[NPROC]; // Hold nodes for queue on stack 
...
static queue crr_queue; // Queue for Compensated Round-Robin (CRR) Scheduler, only holds ronnable processes
...
struct proc* dequeue(){
  if(crr_queue.head == NULL)
    return NULL;
  struct proc* head_proc = crr_queue.head->process;
  crr_queue.head->process = NULL; // Free head node;
...
userinit(void)
{
  ...
  // Add to Compensated Round-Robin scheduler 
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Checking that queue isn't empty
    if(p == NULL){
        release(&ptable.lock);
        continue;
    }
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == SLEEPING && p->chan == chan){
      // Case 1: Special wakeup to syscall sleeping procs via trap every 1 tick
