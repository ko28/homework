...
void handle_alias(char** args, int len) {
  ...
  if (len == 1) {
    ...
    // Case 2: "alias <word>"
    // If the word matches a current alias-name, print the alias-name and
    // corresponding replacement value If the word does not match a current
    // alias-name, just continue
    ...
    // Set up an alias between the alias-name and the value (e.g. alias ll
    // /bin/ls -l -a). If the alias-name was already being used, just replace
...
void handle_unalias(char* alias) { delete (alias); }

void handle_line(const char* userline) {
  if (strlen(userline) <= 0) return;
  ...
  for (int j = 0; j < i; j++) {
    ...
    // (> exists)
    if (strcmp(">", args[j]) == 0) {
      // (exists command to redirect) and (exactly 1 param after >) and (next
      // param is not >)
      if ((j > 0) && (j + 1 == i - 1) && (strcmp(">", args[j + 1]) != 0)) {
        std_out = dup(STDOUT_FILENO);
        fp = fopen(args[j + 1],
                   "w");  // discard old contents if exists and create file
