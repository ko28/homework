...
cat(int fd)
{
  ...
  43:	6a 01                	push   $0x1
  45:	e8 8f 03 00 00       	call   3d9 <printf>
      exit();
  ...
  5d:	68 a6 06 00 00       	push   $0x6a6
  62:	6a 01                	push   $0x1
  64:	e8 70 03 00 00       	call   3d9 <printf>
    exit();
...
main(int argc, char *argv[])
{
  ...
  e2:	ff 37                	pushl  (%edi)
  e4:	68 b7 06 00 00       	push   $0x6b7
  e9:	6a 01                	push   $0x1
  eb:	e8 e9 02 00 00       	call   3d9 <printf>
      exit();
...
putc(int fd, char c)
{
 ...
 34d:	52                   	push   %edx
 34e:	50                   	push   %eax
 34f:	e8 53 ff ff ff       	call   2a7 <write>
}
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 368:	0f 95 c3             	setne  %bl
 36b:	89 d0                	mov    %edx,%eax
 36d:	c1 e8 1f             	shr    $0x1f,%eax
 370:	84 c3                	test   %al,%bl
 372:	74 10                	je     384 <printint+0x2b>
    neg = 1;
    x = -xx;
 374:	f7 da                	neg    %edx
    neg = 1;
 ...
 382:	eb 0b                	jmp    38f <printint+0x36>
  neg = 0;
 384:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)
 38b:	eb f0                	jmp    37d <printint+0x24>
  do{
    buf[i++] = digits[x % base];
 ...
 39a:	8d 46 01             	lea    0x1(%esi),%eax
 39d:	0f b6 92 d4 06 00 00 	movzbl 0x6d4(%edx),%edx
 3a4:	88 54 35 d8          	mov    %dl,-0x28(%ebp,%esi,1)
  }while((x /= base) != 0);
 3a8:	89 da                	mov    %ebx,%edx
 3aa:	85 db                	test   %ebx,%ebx
 3ac:	75 df                	jne    38d <printint+0x34>
 3ae:	89 c3                	mov    %eax,%ebx
  if(neg)
 3b0:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
 3b4:	74 16                	je     3cc <printint+0x73>
    buf[i++] = '-';
 3b6:	c6 44 05 d8 2d       	movb   $0x2d,-0x28(%ebp,%eax,1)
 3bb:	8d 5e 02             	lea    0x2(%esi),%ebx
 3be:	eb 0c                	jmp    3cc <printint+0x73>

  while(--i >= 0)
    putc(fd, buf[i]);
 3c0:	0f be 54 1d d8       	movsbl -0x28(%ebp,%ebx,1),%edx
 3c5:	89 f8                	mov    %edi,%eax
 3c7:	e8 73 ff ff ff       	call   33f <putc>
  while(--i >= 0)
 3cc:	83 eb 01             	sub    $0x1,%ebx
 3cf:	79 ef                	jns    3c0 <printint+0x67>
}
...
printf(int fd, const char *fmt, ...)
{
 ...
 3f9:	e8 41 ff ff ff       	call   33f <putc>
 ...
 411:	0f 84 23 01 00 00    	je     53a <printf+0x161>
    c = fmt[i] & 0xff;
 ...
 42d:	74 49                	je     478 <printf+0x9f>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
 42f:	83 f8 78             	cmp    $0x78,%eax
 432:	0f 94 c1             	sete   %cl
 435:	83 f8 70             	cmp    $0x70,%eax
 438:	0f 94 c2             	sete   %dl
 43b:	08 d1                	or     %dl,%cl
 43d:	75 63                	jne    4a2 <printf+0xc9>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
 43f:	83 f8 73             	cmp    $0x73,%eax
 442:	0f 84 84 00 00 00    	je     4cc <printf+0xf3>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
 ...
 44b:	0f 84 b7 00 00 00    	je     508 <printf+0x12f>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
 451:	83 f8 25             	cmp    $0x25,%eax
 454:	0f 84 cc 00 00 00    	je     526 <printf+0x14d>
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
 45a:	ba 25 00 00 00       	mov    $0x25,%edx
 45f:	8b 45 08             	mov    0x8(%ebp),%eax
 462:	e8 d8 fe ff ff       	call   33f <putc>
        putc(fd, c);
 467:	89 fa                	mov    %edi,%edx
 469:	8b 45 08             	mov    0x8(%ebp),%eax
 46c:	e8 ce fe ff ff       	call   33f <putc>
      }
      state = 0;
 471:	be 00 00 00 00       	mov    $0x0,%esi
 476:	eb 8d                	jmp    405 <printf+0x2c>
        printint(fd, *ap, 10, 1);
 ...
 48a:	e8 ca fe ff ff       	call   359 <printint>
        ap++;
 ...
 49d:	e9 63 ff ff ff       	jmp    405 <printf+0x2c>
        printint(fd, *ap, 16, 0);
 ...
 4b4:	e8 a0 fe ff ff       	call   359 <printint>
        ap++;
 ...
 4c7:	e9 39 ff ff ff       	jmp    405 <printf+0x2c>
        s = (char*)*ap;
 ...
 4d9:	75 28                	jne    503 <printf+0x12a>
          s = "(null)";
 4db:	be cc 06 00 00       	mov    $0x6cc,%esi
 4e0:	8b 7d 08             	mov    0x8(%ebp),%edi
 4e3:	eb 0d                	jmp    4f2 <printf+0x119>
          putc(fd, *s);
 4e5:	0f be d2             	movsbl %dl,%edx
 4e8:	89 f8                	mov    %edi,%eax
 4ea:	e8 50 fe ff ff       	call   33f <putc>
          s++;
 4ef:	83 c6 01             	add    $0x1,%esi
        while(*s != 0){
 4f2:	0f b6 16             	movzbl (%esi),%edx
 4f5:	84 d2                	test   %dl,%dl
 4f7:	75 ec                	jne    4e5 <printf+0x10c>
      state = 0;
 4f9:	be 00 00 00 00       	mov    $0x0,%esi
 4fe:	e9 02 ff ff ff       	jmp    405 <printf+0x2c>
 503:	8b 7d 08             	mov    0x8(%ebp),%edi
 506:	eb ea                	jmp    4f2 <printf+0x119>
        putc(fd, *ap);
 508:	8b 7d e4             	mov    -0x1c(%ebp),%edi
 50b:	0f be 17             	movsbl (%edi),%edx
 50e:	8b 45 08             	mov    0x8(%ebp),%eax
 511:	e8 29 fe ff ff       	call   33f <putc>
        ap++;
 516:	83 c7 04             	add    $0x4,%edi
 519:	89 7d e4             	mov    %edi,-0x1c(%ebp)
      state = 0;
 51c:	be 00 00 00 00       	mov    $0x0,%esi
 521:	e9 df fe ff ff       	jmp    405 <printf+0x2c>
        putc(fd, c);
 526:	89 fa                	mov    %edi,%edx
 528:	8b 45 08             	mov    0x8(%ebp),%eax
 52b:	e8 0f fe ff ff       	call   33f <putc>
      state = 0;
 530:	be 00 00 00 00       	mov    $0x0,%esi
 535:	e9 cb fe ff ff       	jmp    405 <printf+0x2c>
    }
  }
}
...
free(void *ap)
{
 ...
 54e:	a1 c0 09 00 00       	mov    0x9c0,%eax
 553:	eb 02                	jmp    557 <free+0x15>
 555:	89 d0                	mov    %edx,%eax
 557:	39 c8                	cmp    %ecx,%eax
 559:	73 04                	jae    55f <free+0x1d>
 55b:	39 08                	cmp    %ecx,(%eax)
 55d:	77 12                	ja     571 <free+0x2f>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
 55f:	8b 10                	mov    (%eax),%edx
 561:	39 c2                	cmp    %eax,%edx
 563:	77 f0                	ja     555 <free+0x13>
 565:	39 c8                	cmp    %ecx,%eax
 567:	72 08                	jb     571 <free+0x2f>
 569:	39 ca                	cmp    %ecx,%edx
 56b:	77 04                	ja     571 <free+0x2f>
 56d:	89 d0                	mov    %edx,%eax
 56f:	eb e6                	jmp    557 <free+0x15>
      break;
 ...
 58a:	89 08                	mov    %ecx,(%eax)
  freep = p;
 58c:	a3 c0 09 00 00       	mov    %eax,0x9c0
}
 591:	5b                   	pop    %ebx
 592:	5e                   	pop    %esi
 593:	5f                   	pop    %edi
...
morecore(uint nu)
{
 ...
 5c0:	77 05                	ja     5c7 <morecore+0x15>
    nu = 4096;
 ...
 5d1:	50                   	push   %eax
 5d2:	e8 38 fd ff ff       	call   30f <sbrk>
  if(p == (char*)-1)
 5d7:	83 c4 10             	add    $0x10,%esp
 5da:	83 f8 ff             	cmp    $0xffffffff,%eax
 5dd:	74 1c                	je     5fb <morecore+0x49>
    return 0;
  hp = (Header*)p;
  hp->s.size = nu;
 ...
 5e8:	50                   	push   %eax
 5e9:	e8 54 ff ff ff       	call   542 <free>
  return freep;
 5ee:	a1 c0 09 00 00       	mov    0x9c0,%eax
 ...
 600:	eb f4                	jmp    5f6 <morecore+0x44>
...
malloc(uint nbytes)
{
 ...
 60c:	8d 58 07             	lea    0x7(%eax),%ebx
 60f:	c1 eb 03             	shr    $0x3,%ebx
 612:	83 c3 01             	add    $0x1,%ebx
  if((prevp = freep) == 0){
 615:	8b 0d c0 09 00 00    	mov    0x9c0,%ecx
 61b:	85 c9                	test   %ecx,%ecx
 61d:	74 04                	je     623 <malloc+0x21>
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 61f:	8b 01                	mov    (%ecx),%eax
 621:	eb 4d                	jmp    670 <malloc+0x6e>
    base.s.ptr = freep = prevp = &base;
 623:	c7 05 c0 09 00 00 c4 	movl   $0x9c4,0x9c0
 62a:	09 00 00 
 62d:	c7 05 c4 09 00 00 c4 	movl   $0x9c4,0x9c4
 634:	09 00 00 
    base.s.size = 0;
 637:	c7 05 c8 09 00 00 00 	movl   $0x0,0x9c8
 63e:	00 00 00 
    base.s.ptr = freep = prevp = &base;
 641:	b9 c4 09 00 00       	mov    $0x9c4,%ecx
 646:	eb d7                	jmp    61f <malloc+0x1d>
    if(p->s.size >= nunits){
      if(p->s.size == nunits)
 648:	39 da                	cmp    %ebx,%edx
 64a:	74 1a                	je     666 <malloc+0x64>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
 ...
 654:	89 58 04             	mov    %ebx,0x4(%eax)
      ...
      freep = prevp;
 657:	89 0d c0 09 00 00    	mov    %ecx,0x9c0
      return (void*)(p + 1);
 ...
 675:	73 d1                	jae    648 <malloc+0x46>
    if(p == freep)
 677:	39 05 c0 09 00 00    	cmp    %eax,0x9c0
 67d:	75 ed                	jne    66c <malloc+0x6a>
      if((p = morecore(nunits)) == 0)
 67f:	89 d8                	mov    %ebx,%eax
 681:	e8 2c ff ff ff       	call   5b2 <morecore>
 686:	85 c0                	test   %eax,%eax
 688:	75 e2                	jne    66c <malloc+0x6a>
        return 0;
 68a:	b8 00 00 00 00       	mov    $0x0,%eax
 68f:	eb cf                	jmp    660 <malloc+0x5e>
