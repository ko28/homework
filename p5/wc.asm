...
wc(int fd, char *name)
{
  ...
  a6:	68 12 07 00 00       	push   $0x712
  ab:	6a 01                	push   $0x1
  ad:	e8 91 03 00 00       	call   443 <printf>
}
...
main(int argc, char *argv[])
{
 ...
 14c:	ff 37                	pushl  (%edi)
 14e:	68 1f 07 00 00       	push   $0x71f
 153:	6a 01                	push   $0x1
 155:	e8 e9 02 00 00       	call   443 <printf>
      exit();
...
putc(int fd, char c)
{
 ...
 3b7:	52                   	push   %edx
 3b8:	50                   	push   %eax
 3b9:	e8 53 ff ff ff       	call   311 <write>
}
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3d2:	0f 95 c3             	setne  %bl
 3d5:	89 d0                	mov    %edx,%eax
 3d7:	c1 e8 1f             	shr    $0x1f,%eax
 3da:	84 c3                	test   %al,%bl
 3dc:	74 10                	je     3ee <printint+0x2b>
    neg = 1;
    x = -xx;
 3de:	f7 da                	neg    %edx
    neg = 1;
 ...
 3ec:	eb 0b                	jmp    3f9 <printint+0x36>
  neg = 0;
 3ee:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)
 3f5:	eb f0                	jmp    3e7 <printint+0x24>
  do{
    buf[i++] = digits[x % base];
 ...
 404:	8d 46 01             	lea    0x1(%esi),%eax
 407:	0f b6 92 3c 07 00 00 	movzbl 0x73c(%edx),%edx
 40e:	88 54 35 d8          	mov    %dl,-0x28(%ebp,%esi,1)
  }while((x /= base) != 0);
 412:	89 da                	mov    %ebx,%edx
 414:	85 db                	test   %ebx,%ebx
 416:	75 df                	jne    3f7 <printint+0x34>
 418:	89 c3                	mov    %eax,%ebx
  if(neg)
 41a:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
 41e:	74 16                	je     436 <printint+0x73>
    buf[i++] = '-';
 420:	c6 44 05 d8 2d       	movb   $0x2d,-0x28(%ebp,%eax,1)
 425:	8d 5e 02             	lea    0x2(%esi),%ebx
 428:	eb 0c                	jmp    436 <printint+0x73>

  while(--i >= 0)
    putc(fd, buf[i]);
 42a:	0f be 54 1d d8       	movsbl -0x28(%ebp,%ebx,1),%edx
 42f:	89 f8                	mov    %edi,%eax
 431:	e8 73 ff ff ff       	call   3a9 <putc>
  while(--i >= 0)
 436:	83 eb 01             	sub    $0x1,%ebx
 439:	79 ef                	jns    42a <printint+0x67>
}
 43b:	83 c4 2c             	add    $0x2c,%esp
 43e:	5b                   	pop    %ebx
 43f:	5e                   	pop    %esi
 440:	5f                   	pop    %edi
 441:	5d                   	pop    %ebp
...
printf(int fd, const char *fmt, ...)
{
 ...
 463:	e8 41 ff ff ff       	call   3a9 <putc>
 ...
 47b:	0f 84 23 01 00 00    	je     5a4 <printf+0x161>
    c = fmt[i] & 0xff;
 ...
 492:	eb db                	jmp    46f <printf+0x2c>
      if(c == 'd'){
 494:	83 f8 64             	cmp    $0x64,%eax
 497:	74 49                	je     4e2 <printf+0x9f>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
 499:	83 f8 78             	cmp    $0x78,%eax
 49c:	0f 94 c1             	sete   %cl
 49f:	83 f8 70             	cmp    $0x70,%eax
 4a2:	0f 94 c2             	sete   %dl
 4a5:	08 d1                	or     %dl,%cl
 4a7:	75 63                	jne    50c <printf+0xc9>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
 4a9:	83 f8 73             	cmp    $0x73,%eax
 4ac:	0f 84 84 00 00 00    	je     536 <printf+0xf3>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
 ...
 4b2:	83 f8 63             	cmp    $0x63,%eax
 4b5:	0f 84 b7 00 00 00    	je     572 <printf+0x12f>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
 4bb:	83 f8 25             	cmp    $0x25,%eax
 4be:	0f 84 cc 00 00 00    	je     590 <printf+0x14d>
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
 4c4:	ba 25 00 00 00       	mov    $0x25,%edx
 4c9:	8b 45 08             	mov    0x8(%ebp),%eax
 4cc:	e8 d8 fe ff ff       	call   3a9 <putc>
        putc(fd, c);
 4d1:	89 fa                	mov    %edi,%edx
 4d3:	8b 45 08             	mov    0x8(%ebp),%eax
 4d6:	e8 ce fe ff ff       	call   3a9 <putc>
      }
      state = 0;
 4db:	be 00 00 00 00       	mov    $0x0,%esi
 4e0:	eb 8d                	jmp    46f <printf+0x2c>
        printint(fd, *ap, 10, 1);
 4e2:	8b 7d e4             	mov    -0x1c(%ebp),%edi
 4e5:	8b 17                	mov    (%edi),%edx
 4e7:	83 ec 0c             	sub    $0xc,%esp
 4ea:	6a 01                	push   $0x1
 4ec:	b9 0a 00 00 00       	mov    $0xa,%ecx
 4f1:	8b 45 08             	mov    0x8(%ebp),%eax
 4f4:	e8 ca fe ff ff       	call   3c3 <printint>
        ap++;
 ...
 507:	e9 63 ff ff ff       	jmp    46f <printf+0x2c>
        printint(fd, *ap, 16, 0);
 ...
 51e:	e8 a0 fe ff ff       	call   3c3 <printint>
        ap++;
 ...
 531:	e9 39 ff ff ff       	jmp    46f <printf+0x2c>
        s = (char*)*ap;
...
free(void *ap)
{
 ...
 5b8:	a1 20 0a 00 00       	mov    0xa20,%eax
 5bd:	eb 02                	jmp    5c1 <free+0x15>
 5bf:	89 d0                	mov    %edx,%eax
 5c1:	39 c8                	cmp    %ecx,%eax
 5c3:	73 04                	jae    5c9 <free+0x1d>
 5c5:	39 08                	cmp    %ecx,(%eax)
 5c7:	77 12                	ja     5db <free+0x2f>
    if(p >= p->s.ptr && (bp > p || bp < p->s.ptr))
 5c9:	8b 10                	mov    (%eax),%edx
 5cb:	39 c2                	cmp    %eax,%edx
 5cd:	77 f0                	ja     5bf <free+0x13>
 5cf:	39 c8                	cmp    %ecx,%eax
 5d1:	72 08                	jb     5db <free+0x2f>
 5d3:	39 ca                	cmp    %ecx,%edx
 5d5:	77 04                	ja     5db <free+0x2f>
 5d7:	89 d0                	mov    %edx,%eax
 5d9:	eb e6                	jmp    5c1 <free+0x15>
      break;
 ...
 5f4:	89 08                	mov    %ecx,(%eax)
  freep = p;
 5f6:	a3 20 0a 00 00       	mov    %eax,0xa20
}
 5fb:	5b                   	pop    %ebx
 5fc:	5e                   	pop    %esi
 5fd:	5f                   	pop    %edi
...
morecore(uint nu)
{
 ...
 63b:	50                   	push   %eax
 63c:	e8 38 fd ff ff       	call   379 <sbrk>
  if(p == (char*)-1)
 ...
 652:	50                   	push   %eax
 653:	e8 54 ff ff ff       	call   5ac <free>
  return freep;
 658:	a1 20 0a 00 00       	mov    0xa20,%eax
...
malloc(uint nbytes)
{
 ...
 676:	8d 58 07             	lea    0x7(%eax),%ebx
 679:	c1 eb 03             	shr    $0x3,%ebx
 67c:	83 c3 01             	add    $0x1,%ebx
  if((prevp = freep) == 0){
 67f:	8b 0d 20 0a 00 00    	mov    0xa20,%ecx
 685:	85 c9                	test   %ecx,%ecx
 687:	74 04                	je     68d <malloc+0x21>
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 689:	8b 01                	mov    (%ecx),%eax
 68b:	eb 4d                	jmp    6da <malloc+0x6e>
    base.s.ptr = freep = prevp = &base;
 68d:	c7 05 20 0a 00 00 24 	movl   $0xa24,0xa20
 694:	0a 00 00 
 697:	c7 05 24 0a 00 00 24 	movl   $0xa24,0xa24
 69e:	0a 00 00 
    base.s.size = 0;
 6a1:	c7 05 28 0a 00 00 00 	movl   $0x0,0xa28
 6a8:	00 00 00 
    base.s.ptr = freep = prevp = &base;
 6ab:	b9 24 0a 00 00       	mov    $0xa24,%ecx
 6b0:	eb d7                	jmp    689 <malloc+0x1d>
    if(p->s.size >= nunits){
      if(p->s.size == nunits)
 6b2:	39 da                	cmp    %ebx,%edx
 6b4:	74 1a                	je     6d0 <malloc+0x64>
        prevp->s.ptr = p->s.ptr;
      else {
        p->s.size -= nunits;
 6b6:	29 da                	sub    %ebx,%edx
 6b8:	89 50 04             	mov    %edx,0x4(%eax)
        p += p->s.size;
 6bb:	8d 04 d0             	lea    (%eax,%edx,8),%eax
        p->s.size = nunits;
 6be:	89 58 04             	mov    %ebx,0x4(%eax)
      }
      freep = prevp;
 6c1:	89 0d 20 0a 00 00    	mov    %ecx,0xa20
      return (void*)(p + 1);
 ...
 6d4:	eb eb                	jmp    6c1 <malloc+0x55>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 6d6:	89 c1                	mov    %eax,%ecx
 6d8:	8b 00                	mov    (%eax),%eax
    if(p->s.size >= nunits){
 6da:	8b 50 04             	mov    0x4(%eax),%edx
 6dd:	39 da                	cmp    %ebx,%edx
 6df:	73 d1                	jae    6b2 <malloc+0x46>
    if(p == freep)
 6e1:	39 05 20 0a 00 00    	cmp    %eax,0xa20
 6e7:	75 ed                	jne    6d6 <malloc+0x6a>
      if((p = morecore(nunits)) == 0)
 6e9:	89 d8                	mov    %ebx,%eax
 6eb:	e8 2c ff ff ff       	call   61c <morecore>
 6f0:	85 c0                	test   %eax,%eax
 6f2:	75 e2                	jne    6d6 <malloc+0x6a>
        return 0;
 6f4:	b8 00 00 00 00       	mov    $0x0,%eax
 6f9:	eb cf                	jmp    6ca <malloc+0x5e>
